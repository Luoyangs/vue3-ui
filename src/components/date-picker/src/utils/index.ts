 ...
 * @returns Date
 */
export const getStartDateOfCalendar = (year: number, month: number, startOfWeek = 0): Date => {
  const date = new Date(year, month, 1);
 ...
 * @returns boolean
 */
export const isInRange2 = (date: Date, start: Date, end: Date, include = false): boolean => {
  if (!date || !start || !end) {
    return false;
  }
 ...
 */
export const getWeekRange = (date: Date, startOfWeek = 0): Date[] => {
  // 获取当前日期偏离当前周第一天的天数
